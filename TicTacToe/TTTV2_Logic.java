...
public class TTTV2_Logic {
    ...
    public static void checkHoriz(int start) {
        for (int k = 1; k<col_row+1;k++) {
            for(int i = (k-1)*col_row, j = i+1;  j < k*col_row && i < k*col_row; i++, j++) {
                ...
                if(TTTV2.buttons[i].getText() == TTTV2.buttons[j].getText() && (TTTV2.buttons[i].getText() != "" && TTTV2.buttons[j].getText() != "")) {
                    pairH.put(TTTV2.buttons[j].getText(), pairH.get(TTTV2.buttons[j].getText())+1);
    ...
    public static void checkVert(int start) {
        ...
        for (int i = start, j = i+col_row; i<size-1 && j < size; i++, j++) {
            ...
            if(TTTV2.buttons[i].getText() == TTTV2.buttons[j].getText() && (TTTV2.buttons[i].getText() != "" && TTTV2.buttons[j].getText() != "")) {
                pairV.put(TTTV2.buttons[j].getText(), pairV.get(TTTV2.buttons[j].getText())+1);
    ...
    public static void checkDiag(int start) {
        ...
        for (int i = start, j = i+col_row+1; i<size-1 && j < size; i++, j++) {
            int k = i+col_row-1;
            if (k >= col_row) {
                if((TTTV2.buttons[i].getText() == TTTV2.buttons[k].getText() && (TTTV2.buttons[i].getText() != "" && TTTV2.buttons[k].getText() != ""))) {
                    pairDL.put(TTTV2.buttons[k].getText(), pairDL.get(TTTV2.buttons[k].getText())+1);
            ...
            if((TTTV2.buttons[i].getText() == TTTV2.buttons[j].getText() && (TTTV2.buttons[i].getText() != "" && TTTV2.buttons[j].getText() != ""))) {
                pairDR.put(TTTV2.buttons[j].getText(), pairDR.get(TTTV2.buttons[j].getText())+1);
                // TTTV2.buttons[i].setText(TTTV2.buttons[j].getText()+" ");
    ...
    public static void noWindraw() {
        ...
        for (int j = 0; j < size; j++) {
            TTTV2.buttons[j].setBackground(Color.GRAY);
            TTTV2.buttons[j].setEnabled(false);
        }
        TTTV2.txt.setText("DRAW");
        Prompt p = new Prompt();
    ...
    public static void Won(String winner) {
        for (int i = 0; i < size; i++) {
            if (winner == TTTV2.buttons[i].getText()) {
                TTTV2.buttons[i].setBackground(Color.GREEN);
            }
            TTTV2.buttons[i].setEnabled(false);
        }
        TTTV2.txt.setText(winner + " Player wins");
        Prompt p = new Prompt();
